var imgstr = ['1.jpg','2.png','3.png','4.png','5.png'];
...
//столкновение между двумя объектами, по системе один прямоугольник врезался в другой прямоугольник

function cls_ene_bul(i,j)
{
	var x1 = enemies[i].x, y1 = enemies[i].y;
	var x2 = buls[j].x, y2 = buls[j].y;
	//коордианата пули + ее ширина больше чем координаты врага и одновременно координата врага  + его ширина больше чем координата пульки 
	if ( (x2+imgx[2]>x1)&&(x1+imgx[3]>x2)&&(y2+imgy[2]>y1)&&(y1+imgy[3]>y2) )
	{
		//удаление врага и пульки 
		enemies.splice(i,1);
		buls.splice(j,1);
		score+=100;
		return 1;
